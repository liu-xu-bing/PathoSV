...
if args.gene != "None":
    ...
    hg38_gene = pd.read_table("ref_dir/gene_gencodev26_OMIM_GO_info.txt")
if args.exon != "None":
    hg38_exon = pd.read_table(args.exon)
else:
    hg38_exon = pd.read_table("ref_dir/gencode.v26.annotation_exon_info.txt")
...
if args.clinvar != "None":
    ...
    clinvar_info = pd.read_table("ref_dir/clinvar_20241027_sv_info.txt", low_memory=False)
...
if args.tissue != "None":
    ...
    if args.tpm_trans != "None":
        ...
        transcript_tpm = pd.read_table("ref_dir/55tissues_p10_v26_transcript_tpm_mean.txt", low_memory=False)[["ENSG", "ENST", select_tissue]]
    ...
    if len(tmp_clinvar_info) == 0:
        ...
        if len(tmp_clinvar_var_list) == 0:
            ...
            def calculate_overlap_percent(tmp_row):
                return (min([tmp_row.END, tmp_sv.END]) - max([tmp_row.START, tmp_sv.START])) / max([tmp_row.SVLEN, abs(tmp_sv.SVLEN)])
