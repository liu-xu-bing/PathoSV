...
parser.add_argument('--rare', type=str, default = "true", metavar = "", help='Deciding whether output rare sv part which backgound af < 0.01. Default is true.')
parser.add_argument('--backgound', type=str, required=True, metavar = "", help='Background cohort sv allele frequence set for annotation.')
...
# Load data
if spid_list == "None":
    sv_file = pd.read_table(args.sv, low_memory=False)
    if np.unique(sv_file.columns[0:5].isin(["CHROM","START","END","SVTYPE"])).tolist() != [True]:
        logger.error("Incorrected columns names.")
else:
    sample_list = pd.read_table(spid_list, names=["spid"]).spid.tolist()
    sv_file = pd.read_table(args.sv, names=["CHROM","START","END","SVTYPE"] + sample_list, low_memory=False)

background_sv_af = pd.read_table(args.backgound)
