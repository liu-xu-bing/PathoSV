...
if [ ! -f manta_pass_autosome_convertInversion.vcf ];then
	...
	for i in {1..22} X Y
	do
	        awk -v chr="chr$i" 'BEGIN{FS="\t";OFS="\t"}{if($1==chr)print $0}' $manta_file_name >>manta_pass_autosome.vcf
	done
...
if [ -f $sv_raw_dir"/"$spid"/paragraph_max_svlen200/manta_pass_autosome_max_svlen200_paragraph_gt.vcf" ];then
	python3 $merge_tool -m $sv_raw_dir"/"$spid"/manta/manta_pass_autosome_convertInversion_noheader.vcf" -l $sv_raw_dir"/"$spid"/lumpy/lumpy_del_dup.bed" -c $sv_raw_dir"/"$spid"/cnvnator/cnvnator_info.bed" -p $sv_raw_dir"/"$spid"/paragraph_max_svlen200/manta_pass_autosome_max_svlen200_paragraph_gt.vcf" -o manta_pass_autosome
